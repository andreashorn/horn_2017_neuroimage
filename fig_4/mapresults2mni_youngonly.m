...
options.sides=1;
[~,slice]=ea_writeplanes_lcl(options, mnpt(3),1,V,'off', 0,atlases);
...
for meas=1:length(measures)
    ...
    if varargin{8}(1)
        ...
        for pt=1:size(results.(measures{meas}),1)
            p{meas}(pt)=plot(results.(measures{meas})(pt,1),results.(measures{meas})(pt,2),'*','MarkerEdgeColor','w','MarkerFaceColor','w','MarkerSize',35,'LineWidth',2);
