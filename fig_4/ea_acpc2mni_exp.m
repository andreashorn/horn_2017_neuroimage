function fid=ea_acpc2mni(cfg,leadfig,whichnormmethod,tempfile,output)
...
if cfg.mapmethod
    [PathName,FileName,Ext] = fileparts(output);
...
%     end
    ...
    try
    [fpinsub_mm] = ea_map_coords(fidpoints_vox', '', [directory,'y_ea_normparams.nii'], [directory,options.prefs.prenii_unnormalized],whichnormmethod);
    catch
    ...
    if ~isequal(Vinv.dim,anat.dim)
        ...
        [warpinmni_mm] = ea_map_coords(warpcoord_vox, '', [directory,'y_ea_inv_normparams.nii'], tempfile,whichnormmethod);
